...
from FNO import FNO
from generate_data import generate_darcy_data

def generate_data(samples=500, location="data/darcy_data.npz"):
    nx = 64
    X, Y = generate_darcy_data(n_samples=samples, nx=nx)
    
    # Save data to data/darcy_data.npz
    np.savez(location, X=X, Y=Y)

def train_darcy(data_path, proportion=0.8, epochs=50, modes=12, width=32):
    print("========================================")
    print("      Training FNO on Darcy Flow        ")
    print("========================================")

    # 1. Generate Data
    # The output is (n, nx_full, nx_full) where nx_full = nx + 2
    nx = 64
    nx_full = nx + 2
    ...
    # Repeat for all samples: (samples, nx_full, nx_full, 1)
    x_g = np.repeat(X_G[None, ..., None], X.shape[0], axis=0)
    y_g = np.repeat(Y_G[None, ..., None], X.shape[0], axis=0)
    
    X = X[..., None]
    X_full = np.concatenate([X, x_g, y_g], axis=-1) # shape: (samples, nx_full, nx_full, 3)
    Y = Y[..., None]
    ...
    torch.save(model.state_dict(), "darcy_fno.pth")
    print("Model saved to darcy_fno.pth")

if __name__ == "__main__":
    if not os.path.exists("data"):
        os.makedirs("data")
    if not os.path.exists("data/darcy_data_500.npz"):
        generate_data(samples=500, location="data/darcy_data_500.npz")
    train_darcy(data_path="data/darcy_data_500.npz")
